...
*/
void LerOBJ::FreeModel (struct obj_model_t *mdl)
{
	int i;
 ...
 */
int LerOBJ::MallocModel (struct obj_model_t *mdl)
{
	if (mdl->num_verts)
	{
 ...
 * allocating buffers.
 */
int LerOBJ::FirstPass (FILE *fp, struct obj_model_t *mdl)
{
	int v, t, n;
	char buf[256];
 ...
 */
int LerOBJ::SecondPass (FILE *fp, struct obj_model_t *mdl)
{
 ...
 */
int LerOBJ::ReadOBJModel (const char *filename, struct obj_model_t *mdl)
{
	FILE *fp;
 ...
 * Draw the OBJ model.
 */
void LerOBJ::RenderOBJModel (struct obj_model_t *mdl)
{
	int i, j;
...
void LerOBJ::cleanup(struct obj_model_t &objfile)
{
	FreeModel(&objfile);
}
